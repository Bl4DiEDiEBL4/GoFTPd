...
func main() {
	userdata := flag.String("userdata", "", "DrFTPD v3 userdata root")
	...
	generatedGroups := make(map[string]int)
	...
	ensureGroup := func(name string) string {
		...
		if _, ok := generatedGroups[name]; !ok {
			if gid, ok := existingGroupIDs[name]; ok {
				generatedGroups[name] = gid
			} else {
				generatedGroups[name] = nextGID
				nextGID++
			}
	...
	for _, entry := range entries {
		...
		users = append(users, userRecord{
			Username:     username,
			UID:          uid,
			GID:          generatedGroups[primaryGroup],
			Group:        primaryGroup,
	...
	if info, err := os.Stat(groupsDir); err == nil && info.IsDir() {
		...
		for _, entry := range groupEntries {
			...
			ensureGroup(strings.TrimSuffix(entry.Name(), filepath.Ext(entry.Name())))
		}
	}
	...
	writeGroupFile(filepath.Join(etcDir, "group"), groupOrder, generatedGroups)
	writePasswdFile(filepath.Join(etcDir, "passwd"), users)
	writeGroupRecords(outGroupsDir, groupOrder)
	writeUserRecords(outUsersDir, users)
	writeImportLog(*importLog, usersDir, groupsDir, users, groupOrder)
}

func writeGroupFile(path string, groups []string, groupIDs map[string]int) {
	f, err := os.Create(path)
	must(err)
	defer f.Close()
	w := bufio.NewWriter(f)
	for _, group := range groups {
		_, err = fmt.Fprintf(w, "%s:%s:%d:\n", group, "Imported from DrFTPD v3", groupIDs[group])
		must(err)
	}
	must(w.Flush())
}
...
func writePasswdFile(path string, users []userRecord) {
	...
	for _, user := range users {
		_, err = fmt.Fprintf(w, "%s:%s:%d:%d:drftpd3:/site:/bin/false\n", user.Username, user.PasswdHash, user.UID, user.GID)
		must(err)
	}
	must(w.Flush())
}

func writeGroupRecords(dir string, groups []string) {
	for _, group := range groups {
		path := filepath.Join(dir, group)
		f, err := os.Create(path)
		must(err)
		_, err = fmt.Fprintf(f, "GROUP %s\nSLOTS -1 0 0 0\nGROUPNFO Imported from DrFTPD v3\nSIMULT 0\n", group)
...
func writeUserRecords(dir string, users []userRecord) {
	...
	for _, user := range users {
		...
		must(err)
		w := bufio.NewWriter(f)
		mustf(w, "USER Imported from DrFTPD v3\n")
...
func writeImportLog(path, usersDir, groupsDir string, users []userRecord, groups []string) {
	...
	defer f.Close()
	w := bufio.NewWriter(f)
	mustf(w, "mode: drftpd3\n")
	mustf(w, "users_source: %s\n", usersDir)
	if info, err := os.Stat(groupsDir); err == nil && info.IsDir() {
		mustf(w, "groups_source: %s\n", groupsDir)
	} else {
		mustf(w, "groups_source: (derived from users)\n")
	}
