...
func (p *Plugin) Name() string { return "Banned" }
...
func (p *Plugin) Initialize(config map[string]interface{}) error {
	...
	cfg := plugin.ConfigSection(config, "banned")
	if s, ok := stringConfig(cfg, config, "host", "banned_host"); ok && strings.TrimSpace(s) != "" {
		p.host = strings.TrimSpace(s)
	}
	if n := intConfig(configValue(cfg, config, "port", "banned_port"), p.port); n > 0 {
		p.port = n
	}
	if s, ok := stringConfig(cfg, config, "user", "banned_user"); ok && strings.TrimSpace(s) != "" {
		p.user = strings.TrimSpace(s)
	}
	if s, ok := stringConfig(cfg, config, "password", "banned_password"); ok {
		p.password = s
	}
	if b, ok := boolConfig(configValue(cfg, config, "tls", "banned_tls")); ok {
		p.useTLS = b
	}
	if b, ok := boolConfig(configValue(cfg, config, "insecure_skip_verify", "banned_insecure_skip_verify")); ok {
		p.insecure = b
	}
	if n := intConfig(configValue(cfg, config, "timeout_seconds", "banned_timeout_seconds"), 0); n > 0 {
		p.timeout = time.Duration(n) * time.Second
	}
	if raw, ok := configValueOK(cfg, config, "channels", "banned_channels"); ok {
		p.channels = plugin.ToStringSlice(raw, p.channels)
	}
	if s, ok := stringConfig(cfg, config, "reply_target", "banned_reply_target"); ok && strings.TrimSpace(s) != "" {
...
func (p *Plugin) OnEvent(evt *event.Event) ([]plugin.Output, error) {
	...
	siteArgs := "BANNED"
	if args := strings.TrimSpace(evt.Data["args"]); args != "" {
		siteArgs += " " + args
	}
	lines, err := p.runSITE(siteArgs)
	response := responseText(lines)
	vars := map[string]string{
		"command":  strings.TrimSpace(siteArgs),
		"response": response,
		"line":     response,
		"user":     evt.User,
	...
	if err != nil {
		return p.reply(evt, p.render("BANNEDCMD_ERROR", vars, "BANNED: "+response)), nil
	}
	if len(lines) == 0 {
		return p.reply(evt, p.render("BANNEDCMD_LINE", vars, "BANNED: no output.")), nil
	}
	out := make([]string, 0, len(lines))
	for _, line := range lines {
		vars["line"] = line
		out = append(out, p.render("BANNEDCMD_LINE", vars, "BANNED: "+line))
	}
