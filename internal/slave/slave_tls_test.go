...
func TestBuildMasterTLSConfigRejectsPartialClientCertificate(t *testing.T) {
	for _, s := range []*Slave{
		{masterHost: "127.0.0.1", clientCert: "slave.crt"},
		{masterHost: "127.0.0.1", clientKey: "slave.key"},
	} {
		if _, err := s.buildMasterTLSConfig(); err == nil {
