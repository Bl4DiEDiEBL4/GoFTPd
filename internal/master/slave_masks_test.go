...
func TestSlaveManagerMasksAddRemoveList(t *testing.T) {
	...
	if masks := sm.ListSlaveMasks("slave1"); len(masks) != 1 {
		t.Fatalf("expected slave mask names to be case-insensitive, got %v", masks)
	}
...
func TestSlaveMaskMutationRollsBackWhenPersistenceFails(t *testing.T) {
	sm := NewSlaveManager("127.0.0.1", 1099, false, "", "", 60*time.Second)
	dir := t.TempDir()
	maskFile := filepath.Join(dir, "slave_masks.txt")
	if err := sm.ConfigureSlaveMasksFile(maskFile); err != nil {
	...
	if err := sm.AddSlaveMask("SLAVE1", "127.0.0.1"); err != nil {
		t.Fatalf("AddSlaveMask initial: %v", err)
	}
	...
	sm.masks.mu.Lock()
	sm.masks.file = filepath.Join(blocker, "slave_masks.txt")
	sm.masks.mu.Unlock()

	if err := sm.AddSlaveMask("SLAVE1", "127.0.0.2"); err == nil {
		t.Fatal("expected persistence failure")
	}
	if got := sm.ListSlaveMasks("SLAVE1"); len(got) != 1 || got[0] != "127.0.0.1" {
		t.Fatalf("failed add changed runtime masks: %v", got)
	}
	if removed, err := sm.RemoveSlaveMask("SLAVE1", "127.0.0.1"); err == nil || removed {
		t.Fatalf("expected failed remove to report an error, got removed=%v err=%v", removed, err)
	}
	if got := sm.ListSlaveMasks("SLAVE1"); len(got) != 1 || got[0] != "127.0.0.1" {
		t.Fatalf("failed remove changed runtime masks: %v", got)
	}
}
