...
func (sm *SlaveManager) ConfigureSlaveMTLS(caCertPath string) error {
	...
	if sm.running.Load() {
		if caCertPath == sm.slaveCACert {
			return nil
		}
		return fmt.Errorf("master.slave_ca_cert cannot change while the slave listener is running; restart the master")
	}
	if caCertPath == "" {
		sm.slaveCACert = ""
		sm.slaveCAPool = nil
	...
	if !sm.tlsEnabled {
		return fmt.Errorf("master.slave_ca_cert requires tls_enabled: true")
	}
...
func (sm *SlaveManager) Start() error {
	...
	if sm.mTLSActive() && !sm.tlsEnabled {
		return fmt.Errorf("master.slave_ca_cert requires TLS on the slave control listener")
	}
